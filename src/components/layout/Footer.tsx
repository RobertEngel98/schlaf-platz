    ...
    <footer className="bg-sp-blue text-white/50 pt-16 pb-8">
      <div className="wrap">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          <div>
            ...
            <div className="flex gap-3">
              {[
                ...
                <a key={s.l} href={s.href} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-white/[0.08] flex items-center justify-center hover:bg-white/20 transition-colors" aria-label={s.l}>
          ...
          {([
            ...
            <div key={col.t}>
              ...
              {col.links.map(l => <Link key={l.label} href={l.href} className="block text-white/50 no-underline text-sm py-1 hover:text-white transition-colors">{l.label}</Link>)}
            </div>
          ))}
        </div>
        <div className="border-t border-white/[0.08] pt-6 flex flex-wrap justify-between gap-4 text-[13px]">
          ...
          <p>Tel: <a href={`tel:${BRAND.phone}`} className="text-white no-underline">{BRAND.phonePretty}</a></p>
        </div>
      </div>
    </footer>
  );
