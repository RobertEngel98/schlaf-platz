    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${sc ? "bg-[#0a0a0a]/95 backdrop-blur-2xl border-b border-white/[0.06] py-3" : "bg-transparent py-5"}`}>
      <div className="wrap flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-0.5">
          {NAV.map(l => (
            <Link key={l.href} href={l.href} className="no-underline text-[13px] font-medium px-4 py-2 rounded-lg transition-all duration-200 text-white/50 hover:text-white hover:bg-white/[0.06]">
              {l.label}
            </Link>
          ))}
          <Link href="/mieter" className="ml-4 cta-green !text-[13px] !px-5 !py-2.5">
            Jetzt anfragen
          </Link>
        </nav>
      ...
      {op && (
        <div className="md:hidden bg-[#111] animate-slide-down">
          <div className="wrap py-4 border-t border-white/[0.06]">
            {NAV.map(l => (
              <Link key={l.href} href={l.href} className="block text-white/70 no-underline py-3.5 text-[15px] font-medium border-b border-white/[0.06] last:border-0" onClick={() => setOp(false)}>
                {l.label}
              </Link>
            ))}
            <Link href="/mieter" className="cta-green mt-4 w-full text-center">Jetzt anfragen</Link>
          </div>
        </div>
      )}
    </header>
