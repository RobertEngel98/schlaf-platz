...
export default async function stundenerfassungRoutes(app: FastifyInstance) {
  ...
  app.get("/api/stundenerfassung", async (req) => {
    const { page = "1", limit = "25", userId = "", fuerWen = "" } = req.query as Record<string, string>;
    ...
    const [data, [{ total }]] = await Promise.all([
      db.select().from(schema.stundenerfassung).where(where).orderBy(desc(schema.stundenerfassung.datum)).limit(parseInt(limit)).offset(offset),
  ...
  // SUMMARY - Stunden pro Monat/User
